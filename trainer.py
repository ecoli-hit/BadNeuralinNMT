...
class Trainer(object):
    ...
    def calculate_fisher(self,samples):
            '''
            ...
            gradient_mask = dict()
            param_clean = dict()
            param_statistic = dict()
            
            for total ,  params in zip(model.named_parameters(),model.parameters()):
                name = total[0]
                if 'layer' in name:
                    param_statistic[name] = 0       
                    gradient_mask[params] = params.new_zeros(params.size())
                    param_clean[params] = params.new_zeros(params.size())
            ...
            for i ,sample in enumerate(samples):
                ...
                for name , params in model.named_parameters():
                    if 'layer' in name:
                        gradient_mask[params] = gradient_mask[params] + (params.grad ** 2)
                        param_clean[params] = param_clean[params] + (params.grad ** 2)
                del loss 
            
            
            return gradient_mask ,param_clean , param_statistic
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("optimizer"):
                ...
                if hasattr(self.optimizer.optimizer,'maskdone') and self.optimizer.optimizer.maskdone == False :
                    # print(self.optimizer.optimizer.maskdone)
                    if self.optimizer.optimizer.maskdone == False:
                        gradient_mask,param_clean ,param_statistic= self.calculate_fisher(samples)
                        self.optimizer.optimizer.param_statistic_up = param_statistic.copy()
                        self.optimizer.optimizer.param_statistic_down = param_statistic.copy()
                        if self.optimizer.optimizer.gradient_mask == None:
                            self.optimizer.optimizer.gradient_mask = gradient_mask
                            self.optimizer.optimizer.param_clean   = param_clean
                        else:
                            for key in self.optimizer.optimizer.gradient_mask:
                                self.optimizer.optimizer.gradient_mask[key] += gradient_mask[key]
                                self.optimizer.optimizer.param_clean[key] += param_clean[key]
